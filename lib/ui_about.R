...
#observeEvent(input$btn_goto_app, {
#  updateNavbarPage( session, "landing_t", selected = "app_t")
#})

observeEvent(input$btn_goto_app, {
  appendTab(inputId = "nav.main",
...
output$ui.about <- renderUI({
#  btn_credits <- bsButton("btn_credits", "here", style="primary", size="extra-small", class="wb-btn-inline")
  out <- fluidRow(
    column(width = 8, offset = 2, h2(("CSVDriver Documentation"))),
    column(width = 8, offset = 2, h4(("About CSVDriver"))),
    column(width = 8, offset = 2, p("This graphical user interface of",code("CSVDriver")," method that allows the analyse of the structuarl variation in cancers predicting the potential drivers in regions of significant clustering of breakpoint.
    It is not nesserary knowing in the",code("R"),"programming language to use it. Detailed information on how to use this graphical user-interface (GUI) can be found in a tutorial (a so-called vignette) that is included in the",code("CSVDriver"),"package.
    The vignette is available on",tags$a("GitHub pages", href="https://sdctools.github.io/sdcMicro/articles/sdcMicro.html", target="_blank"),
                                    "and via the", tags$a("CRAN", href = "https://cran.r-project.org/web/packages/sdcMicro/vignettes/sdcApp.html"), "website."))#,
...
#  if (is.null(inputdata())) {
    btn <- bsButton(paste0("btn_goto_app"), label=("this button"), style="primary", size="extra-small", class="wb-btn-inline")
...
#  stop_btn <- bsButton("stop_sdcApp", label="Stop the GUI", style="primary", size="extra-small", class="wb-btn-inline", onclick="setTimeout(function(){window.close();},500);")
  out <- list(out, fluidRow(
#    column(width = 8, offset = 2, h4(("QuickStart"))),
...
#                                    "will contain the micro data and the sdc problem at the state just before stopping the GUI.")),
  ...
  out <- list(out, fluidRow(
    ...
    column(width = 8, offset = 2, p("In case you have any suggestions or bug reports, please file an issue at the",
                                    tags$a("issue tracker", href="https://github.com/khuranalab/CSVDriver/issues", target="_blank"),"in our",
                                    tags$a("GitHub repo", href="https://github.com/khuranalab/CSVDriver", target="_blank"),"."))#,
